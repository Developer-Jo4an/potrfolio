...
export function useStar() {
  ...
  return (stars, {x: xStart, y: yStart}, to) => {
    ...
    if (tweenRefs.current.star) tweenRefs.current.star.kill();
    ...
    stars.forEach((star) => gsap.set(star, {x: 0, y: 0, rotate: 0, left: xStart, top: yStart, opacity: 0}));
    ...
    stars.forEach((star, index, {length}) => {
      starTween
        ...
        .to(
          star,
          {motionPath: {path: [start, middle, end], curviness: 1.75, autoRotate: false}, duration: 1, ease: "none"},
          "<",
        )
        .to(star, {opacity: 0, duration: 0.1, ease: "none"}, "-=0.1")
        .to(star, {rotate: 720, duration: 1, ease: "sine.inOut"}, 0);
    });
  };
}
