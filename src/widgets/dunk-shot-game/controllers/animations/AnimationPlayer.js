...
class DunkShotAnimationPlayer extends AnimationPlayer {
  ...
  basketNextAnimation(basket, isImmediate) {
    ...
    nextTween.to(view, {pixi: {scale: 1, alpha: 1}, ease: "back.out", duration: 0.4});

    return new Promise((res) =>
      nextTween.eventCallback("onComplete", () => {
  ...
  basketRotationAnimation(basket) {
    const {
      config: {
        configuration: {
          global_basket: {swing_angle},
        },
      },
      storage: {
        mainSceneSettings: {
          basket: {
            rotation: {speedIntervals},
          },
        },
      },
    } = this;
    ...
    const rotationTween = gsap.timeline({repeat: -1}).save(DUNK_SHOT_TWEEN, `basketRotation${_factoryUUID}`);

    const fullDuration = speedIntervals[speed - 1];

    rotationTween
      .to(basket, {angle: radiansSwingAngle, duration: fullDuration / 4, ease: "none"})
      .to(basket, {angle: -radiansSwingAngle, duration: fullDuration / 2, ease: "none"})
      .to(basket, {angle: 0, duration: fullDuration / 4, ease: "none"});
  }
  ...
  basketInactiveAnimation(basket, isImmediate) {
    ...
    inactiveTween.to(view, {pixi: {scale: 0, alpha: 0}, ease: "back.in", duration: 0.4});

    return new Promise((res) =>
      inactiveTween.eventCallback("onComplete", () => {
  ...
  basketCaughtAnimation(basket, velocity, onUpdate) {
    ...
    const prevData = {scaleY: basketGridFront.scale.y};
    ...
    caughtAnimation
      .to(basketGridFront.scale, {y: prevData.scaleY * caught.degreeOfStretch, ease: "sine.out", duration: 0.075})
      .to(basketGridFront.scale, {y: prevData.scaleY, ease: "sine.out", duration: 0.075})
      .to(
        basket,
        {
          angle: targetAngle,
    ...
    return new Promise((res) =>
      caughtAnimation.eventCallback("onComplete", () => {
  ...
  basketDefaultAnimation(basket, {alpha, rotation, scale} = {}) {
    const {
      storage: {
        mainSceneSettings: {
          basket: {gridBack, back, gridFront, front},
        },
      },
    } = this;
    ...
    if (alpha) {
      if (alpha?.isImmediate) view.alpha = 1;
      else defaultTween.to(view, {alpha: 1, duration: 0.3, ease: "sine.inOut"});
    }
    ...
    if (rotation) {
      ...
      if (rotation?.isImmediate) basket.angle = targetAngle;
      else defaultTween.to(basket, {angle: targetAngle, duration: 0.3, ease: "sine.inOut"}, "<");
    }
    ...
    if (scale) {
      ...
      if (scale.isImmediate) totalScalesData.forEach(({element, scale}) => element.scale.set(scale));
      else
        defaultTween.to(
          totalScalesData.map(({element}) => element),
          {pixi: {scale: (i) => totalScalesData[i].scale}, duration: 0.3, ease: "sine.inOut"},
          "<",
        );
    }

    return new Promise((res) =>
      defaultTween.eventCallback("onComplete", () => {
  ...
  ballHideAnimation(ball) {
    ...
    hideTween.to(view, {alpha: 0, ease: "sine.inOut", duration: 0.3});
  ...
  ballShowAnimation(ball) {
    const {
      ...
      _factoryUUID,
      storage: {
        mainSceneSettings: {ball: ballSettings},
      },
    } = ball;

    const startData = {scale: {x: view.scale.x / 2, y: view.scale.y / 2}};
    ...
    showTween.to(view, {pixi: fields, ease: "back.out", duration: 0.4});

    return new Promise((res) =>
      showTween.eventCallback("onComplete", () => {
  ...
  ballMagnetToBasket(ball, x, y, velocity) {
    ...
    magnetTween.to(proxyObject, {x, y, ease: "sine.out", duration: multiplier / velocity});

    return new Promise((res) =>
      magnetTween.eventCallback("onComplete", () => {
  ...
  ballDamageAnimation(ball) {
    ...
    const damageTween = gsap
      .to(view, {tint: 0xff0000, duration: 0.3, ease: "none", yoyo: true, repeat: -1})
  ...
  finishShowAnimation(finish) {
    ...
    showTween.to(view, {alpha: 1, duration: 0.3, ease: "sine.inOut"});
  ...
  finishHideAnimation(finish) {
    ...
    hideTween.to(view, {alpha: 0, duration: 0.3, ease: "sine.inOut"});
  ...
  spikeInactiveAnimation(spike, isImmediate) {
    ...
    inactiveTween.to(view, {pixi: {scale: 0, alpha: 0}, ease: "back.in", duration: 0.2});
  ...
  spikeShowAnimation(spike) {
    ...
    const fields = {scale: Math.min(width / view.width, height / view.height), alpha: 1};
  ...
  spikeMoveAnimation(spike, availableRoad) {
    ...
    availableRoad.forEach((position, index, {length}) => {
      moveTween.to(spike, {x: position.x, y: position.y, duration: duration / length, ease: "none"});
    });
  }
  ...
  wingsShowAnimation(wings) {
    ...
    const prevData = {scale: {x: view.scale.x, y: view.scale.y}};

    const startData = {scale: {x: view.scale.x / 2, y: view.scale.y / 2}};

    view.scale.set(1);

    const fields = {scaleX: prevData.scale.x, scaleY: prevData.scale.y, alpha: 1};
  ...
  wingsFlyAnimation(wings) {
    ...
    const flyTween = gsap.timeline({repeat: -1}).save(DUNK_SHOT_TWEEN, `wingsFly${_factoryUUID}`);
  ...
  async wingsBoosterAnimation(ball, leftWing, rightWing, activeBasket, nextBasket) {
    const {
      storage: {
        mainSceneSettings: {
          boosters: {
            wings: {
              animation: {offset, wingsAcceleration},
            },
          },
        },
      },
    ...
    const tweenPoint = utils.calculateTweenPoint(activeBasket, nextBasket);
    ...
    const end = {x: utils.getBallTarget(nextBasket).x, y: utils.getBallTarget(nextBasket).y};
    ...
    const pureHitTimeline = gsap
      .timeline({
        onUpdate() {
          ...
          if (
            progress < 0.8 ||
            wings.every(({_factoryUUID}) => gsap.localTimeline.isExist(DUNK_SHOT_TWEEN, `wingsHide${_factoryUUID}`))
          )
            return;

          wings.forEach((wing) => animationPlayer.wingsHideAnimation(wing));
        },
      })
      .save(DUNK_SHOT_TWEEN, "wingsBooster");

    pureHitTimeline
      .to(ball, {x: start.x, y: start.y, ease: "sine.inOut", duration: 0.5})
      .set(
        wings.map(({view}) => view),
        {
          x: (_, element) => {
            ...
            return x + {[LEFT]: offset.wingsStartPosition.left, [RIGHT]: offset.wingsStartPosition.right}[side];
          },
      ...
      .to(ball, {
        motionPath: {path: [start, middle, end], curviness: 2.5, autoRotate: false},
        delay: 0.2,
        duration: 2.25,
        ease: "sine.inOut",
        onStart() {
          wings.forEach(animationPlayer.wingsFlyAnimation);
        },
        onUpdate() {
          const progress = this.progress();
          ...
          wings.map((wing) => {
            ...
            if (animation) animation.timeScale(Math.min(1, progress * wingsAcceleration));
    ...
    await new Promise((res) =>
      pureHitTimeline.eventCallback("onComplete", () => {
        ...
        wings.forEach((wing) => {
          allTweens.forEach((tween) => {
            if (tween.tweenId?.endsWith?.(wing._factoryUUID)) tween.delete(DUNK_SHOT_TWEEN);
          });
        });
...
export const animationPlayer = new DunkShotAnimationPlayer();
