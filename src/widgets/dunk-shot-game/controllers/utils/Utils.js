...
export class DunkShotUtils extends GameUtils {
    ...
    const calculateRoadRecurse = (positionLabel, availableMovement) => {
      for (const place of availableMovement)
        if (movement[positionLabel].includes(place)) {
          road.push(place);
          calculateRoadRecurse(
            place,
            availableMovement.filter((placeLabel) => placeLabel !== place),
          );
        }
    };
  ...
  calculateTweenPoint(activeBasket, nextBasket) {
    ...
    if ([LEFT, RIGHT].includes(nextBasketPositionLabel)) {
      ...
      return {x: GAME_SIZE.width / 2, y: y + GAME_SIZE.height * tweenPoint.yMultiplier};
    }

    const actions = {
      [LEFT]() {
        const {y} = nextBasket;
        return {x: GAME_SIZE.width * tweenPoint.xMultiplier, y: y + GAME_SIZE.height * tweenPoint.yMultiplier};
      },
      [RIGHT]() {
        const {y} = nextBasket;
        return {
...
export const utils = new DunkShotUtils();
