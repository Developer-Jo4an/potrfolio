    ...
    get(target, prop, receiver) {
      ...
      return async function (...args) {
        ...
        if (isFunction(beforeFunction))
          await beforeFunction.call(target);

        if (isFunction(originalFunction))
          await originalFunction.call(target, ...args);
        ...
        return Promise.all(controllers.map(controller => {
          ...
          return isFunction(necessaryStateFunction)
            ? necessaryStateFunction.call(controller)
            : Promise.resolve();
        }));
      };
    }
